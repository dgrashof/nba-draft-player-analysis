...
#load nba basic stastics from basketball reference
nba_df_basic = pd.DataFrame()
for year in range(season_min,season_max):
    temp = pd.DataFrame(i for i in client.players_season_totals(season_end_year=year))
    temp['year'] = year
    nba_df_basic = nba_df_basic.append(temp)

nba_df_basic.drop(columns=['positions','age','minutes_played','games_played'],inplace = True)
nba_df_basic['three_point_percentage'] = nba_df_basic['made_three_point_field_goals']/nba_df_basic['attempted_three_point_field_goals']
nba_df_basic['free_throw_percentage'] = nba_df_basic['made_free_throws']/nba_df_basic['attempted_free_throws']

nba_df = nba_df.merge(right=nba_df_basic,how='inner',on=['slug','name','team','year'])
...
avg_columns = ['assist_percentage','block_percentage','defensive_rebound_percentage','free_throw_attempt_rate',
               ...
               'true_shooting_percentage','three_point_percentage','free_throw_percentage','turnover_percentage','usage_percentage','box_plus_minus']

sum_columns = ['minutes_played','value_over_replacement_player','attempted_three_point_field_goals','attempted_free_throws','steals','assists','blocks','turnovers']
...
nba_df = pd.merge(left = nba_df,right = sum_df[['name','year','total_minutes_played']],how = 'left',on = ['name','year'])
...
#drop old sum columns
nba_df = nba_df.drop(columns = ['value_over_replacement_player','attempted_three_point_field_goals','attempted_free_throws','steals','assists','blocks','turnovers'])
#merge partial rows together
nba_df_final = nba_df.groupby(['name','year']).agg(sum).reset_index()[avg_columns+group_by_columns]
#
nba_df_final = pd.merge(left = nba_df_final,right = sum_df[['name','year','total_minutes_played','value_over_replacement_player','attempted_three_point_field_goals','attempted_free_throws','steals','assists','blocks','turnovers']],how = 'left',on = ['name','year'])
#tack on age
